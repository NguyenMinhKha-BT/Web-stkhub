...
?>
   ...
   $id = $_GET["id"];
   $query="SELECT s.ID,s.Ten,s.date,s.Gia,s.HinhAnh,s.thuonghieu,s.KhuyenMai,s.giakhuyenmai,s.Mota, n.Ten as Tenlinhkien,s.Malinhkien
   from sanpham s 
   LEFT JOIN linhkien n on n.ID = s.Malinhkien
	WHERE  s.id =".$id;
...
?>
					...
					<div class="product">
						...
						<div class="col-md-6">
							<div class="caption">
								...
								<div class="info">
									<ul>
										<li>Thương hiệu: <b><?php echo $row["thuonghieu"]?></b></li>
										<li>Linh kiện máy tính: <a href="/category.php?malinhkien=<?php echo $row["Malinhkien"]?>"><?php echo $row["Tenlinhkien"]?></a> <h3></li>
									</ul>
								</div>
								<?php
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
		...
		<p>Chức năng mua ngay giúp bạn mua nhanh mà không cần đăng nhập hoặc đặt hàng với một thông tin khác với thông tin trên tài khoản. Tuy nhiên bạn chỉ có thể mua một loại sản phẩm trong một lần đặt hàng, bạn nên đăng nhập để không phải nhập lại thông tin cũng như mua nhiều loại sách cùng lúc.</p>
